...
class Evaluator:
    ...
    def eval_mask_transfer(cls, prd_kps, batch):
        ...
        eval_result = {"ltacc": ltacc, "iou": iou}
    ...
    def intersection_over_union(cls, mask1, mask2):
        ...
        rel_part_weight = torch.sum(torch.sum(mask2.gt(0.5).float(), 2, True), 3, True) / torch.sum(
            mask2.gt(0.5).float()
        )
        part_iou = torch.sum(torch.sum((mask1.gt(0.5) & mask2.gt(0.5)).float(), 2, True), 3, True) / torch.sum(
            torch.sum((mask1.gt(0.5) | mask2.gt(0.5)).float(), 2, True), 3, True
        )
