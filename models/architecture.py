...
from models.base.transformer import TransformerEncoderLayer
from models.normalization import SPADE, SPADE_TwoPath, equal_lr
...
class SPADETransformerBlock(nn.Module):
    def __init__(self, fin, fout, nhead) -> None:
        super().__init__()
        fmiddle = min(fin, fout)
        self.learned_shortcut = fin != fout

        self.layer_0 = TransformerEncoderLayer(fin, nhead, attention_type="self")
        self.layer_1 = TransformerEncoderLayer(fmiddle, nhead, attention_type="self")

        self.proj = nn.Conv2d(fin, fmiddle, 1)
        self.proj = spectral_norm(self.proj)

        spade_config_str = "spadesyncbatch3x3"
        ic = 3
        self.norm_0 = SPADE(spade_config_str, fin, ic, PONO=True, use_apex=False)
        self.norm_1 = SPADE(spade_config_str, fmiddle, ic, PONO=True, use_apex=False)

        if self.learned_shortcut:
            self.conv_s = nn.Conv2d(fin, fout, kernel_size=1, bias=False)
            self.conv_s = spectral_norm(self.conv_s)
            self.norm_s = SPADE(spade_config_str, fin, ic, PONO=True, use_apex=False)
    ...
    def forward(self, x, seg1):
        ...
        dx = self.layer_0(self.actvn(self.norm_0(x, seg1)))
        dx = self.proj(dx)
        dx = self.layer_1(self.actvn(self.norm_1(dx, seg1)))
...
class SPADEResnetBlock(nn.Module):
    def __init__(
        self, fin, fout, use_se=False, dilation=1, ic=3,
    ):
        super().__init__()
        # Attributes
        self.learned_shortcut = fin != fout
