    ...
    def __init__(self, config_text, norm_nc, label_nc_example, label_nc_imagine, PONO=False, use_apex=False):
        ...
        if self.pad_type != "zero":
            self.mlp_shared_example = nn.Sequential(
                nn.ReflectionPad2d(pw), nn.Conv2d(label_nc_example, nhidden, kernel_size=ks, padding=0), nn.ReLU()
            )
            self.pad = nn.ReflectionPad2d(pw)
            ...
            self.mlp_shared_imagine = nn.Sequential(
                nn.ReflectionPad2d(pw), nn.Conv2d(label_nc_imagine, nhidden, kernel_size=ks, padding=0), nn.ReLU()
            )
            ...
            self.mlp_shared_example = nn.Sequential(
                nn.Conv2d(label_nc_example, nhidden, kernel_size=ks, padding=pw), nn.ReLU()
            )
            ...
            self.mlp_shared_imagine = nn.Sequential(
                nn.Conv2d(label_nc_imagine, nhidden, kernel_size=ks, padding=pw), nn.ReLU()
            )
