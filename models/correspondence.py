...
from models.generator import AdaptiveFeatureGenerator, DomainClassifier, ReverseLayerF
...
class NoVGGCorrespondence(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        self.layer = nn.Sequential(
            ResidualBlock(
                self.feature_channel * 4 + label_nc + coord_c,
                self.feature_channel * 4 + label_nc + coord_c,
                kernel_size=3,
                padding=1,
                stride=1,
        ...
        self.phi = nn.Conv2d(
            in_channels=self.in_channels + label_nc + coord_c,
    ...
    def forward(
        ...
        if self.opt.isTrain and self.opt.novgg_featpair > 0:
            ...
            coor_out["loss_novgg_featpair"] = (
                F.l1_loss(adaptive_feature_seg, adaptive_feature_img_pair) * self.opt.novgg_featpair
            )
        ...
        if self.opt.maskmix:
            ...
            if self.opt.noise_for_mask and (
                (not self.opt.isTrain) or (self.opt.isTrain and self.opt.epoch > self.opt.mask_epoch)
            ):
        ...
        if self.opt.warp_cycle_w > 0:
            ...
            if self.opt.warp_patch:
                ...
                if self.opt.two_cycle:
                    ...
                    coor_out["warp_i2r2i"] = (
                        warp_i2r2i.permute(0, 2, 1)
                        .contiguous()
                        .view(batch_size, channel, feature_height, feature_width)
                    )
