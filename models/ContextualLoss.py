...
from util.util import fbcheck, feature_normalize, get_flow, mse_loss
...
class ContextualLoss_forward(nn.Module):
    ...
    def forward(self, X_features, Y_features, h=0.1, feature_centering=True):
        ...
        fb_mask = fbcheck(*get_flow(A_ij.reshape(-1, feature_size, feature_size, feature_size, feature_size)))[0]
        # contextual loss per sample
        wta = torch.max(A_ij, dim=-1)[0]
        wta_mask = wta.ge(0.3)
        if wta_mask.sum() == 0:
            loss = torch.tensor([0], device=wta.device).float()
        else:
            CX = torch.mean(torch.masked_select(wta, wta_mask), dim=-1)
            loss = -torch.log(CX)
...
class ChamferDistance_patch_loss(nn.Module):
    ...
    def forward(self, X_features, Y_features, patch_size=3, image_x=None, image_y=None, h=0.1, Y_features_in=None):
        ...
        if image_x is not None and image_y is not None:
            image_x = torch.nn.functional.interpolate(
                image_x, size=(feature_size, feature_size), mode="bilinear"
            ).view(batch_size, 3, -1)
            image_y = torch.nn.functional.interpolate(
                image_y, size=(feature_size, feature_size), mode="bilinear"
            ).view(batch_size, 3, -1)
