...
from models.diffusionmodules.model import Decoder, Encoder
from models.distributions.distributions import DiagonalGaussianDistribution
from models.ema import LitEma
from models.feature_backbones.vqvae import VectorQuantizer2 as VectorQuantizer
...
class VQModel(pl.LightningModule):
    def __init__(
        ...
        self.loss = torch.nn.Identity  # For sampling only
...
class VQModelInterface(VQModel):
    ...
    def encode(self, x, intermediate=False):
        h, inter = self.encoder(x)
        h = self.quant_conv(h)
        if intermediate:
            return h, inter
        else:
