...
class CelebAHQEdgeDataset(Pix2pixDataset):
    ...
    def get_label_tensor(self, path):
        ...
        transform_label = get_transform(self.opt, params, method=T.InterpolationMode.NEAREST, normalize=False)
        transform_img = get_transform(self.opt, params, method=T.InterpolationMode.BILINEAR, normalize=False)
        ...
        for part in inner_parts:
            ...
            if os.path.exists(path.format(part)):
                part_label = (
                    Image.open(path.format(part))
                    .convert("L")
                    .resize((self.opt.load_size, self.opt.load_size), resample=Image.NEAREST)
                )
