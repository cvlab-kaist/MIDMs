...
class DeepFashionDataset(Pix2pixDataset):
    ...
    def get_ref_video_like(self, opt):
        ...
        ref_name = "./data/deepfashion_ref_test.txt" if not opt.is_train else "./data/deepfashion_ref.txt"
        ...
        for i in range(len(ref)):
            ...
            if items[0] in key_name.keys():
                ...
                ref_dict[items[0].replace("\\", "/")] = [items[-1].replace("\\", "/")] + [
                    it.split(split)[-1] for it in random.sample(self.image_paths, min(len(self.image_paths), 20))
                ]
        train_test_folder = ("", "")
    ...
    def get_ref(self, opt):
        # if opt.video_like:
        if False:
            raise Exception("video_like is not supported in DeepFashion dataset")
    ...
    def get_label_tensor(self, path):
        ...
        pose = Image.fromarray(cv2.cvtColor(canvas, cv2.COLOR_BGR2RGB)).resize(
            (self.opt.load_size, self.opt.load_size), resample=Image.NEAREST
        )
        ...
        transform_img = get_transform(self.opt, params, method=InterpolationMode.BILINEAR, normalize=False)
        tensors_dist = 0
        e = 1
        ...
        # save_image(label_tensor, f'./label_tensor.png')
