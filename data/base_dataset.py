...
def get_transform(opt, params, method=Image.BICUBIC, normalize=True, toTensor=True, CATs_aug=False):
    transform_list = []
    if opt.benchmark == "flickr" and method == Image.NEAREST:
    ...
    if opt.is_train and "rotate" in params.keys():
    ...
    if CATs_aug:
        cats_transform_list = A.Compose(
            [
                A.ToGray(p=0),
                A.Posterize(p=0),
                A.Equalize(p=0),
                A.augmentations.transforms.Sharpen(p=0),
                A.RandomBrightnessContrast(p=0),
                A.Solarize(p=0),
                A.ColorJitter(p=0),
            ]
        )
        return transforms.Compose(transform_list), cats_transform_list
    else:
